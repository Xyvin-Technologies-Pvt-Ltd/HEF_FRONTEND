...
import { getEventAttendedUsers, downloadAttendedUsers, getEventById } from "../../api/eventapi";
...
const ParticipantsTable = ({ eventId }) => {
    ...
    const fetchParticipants = async () => {
        setLoading(true);
        try {
            const res = await getEventAttendedUsers(
                eventId,
                pageNo,
                rowPerSize,
                filters?.chapterId
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            const res = await downloadAttendedUsers(eventId, filters?.chapterId);
            if (res?.data?.headers && res?.data?.body) {
                const { body, attendedCount, totalSeats } = res.data;
                ...
                generatePDF(res.data.headers, body, "Participants List", eventInfo);
